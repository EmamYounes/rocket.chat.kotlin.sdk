...
class MessagesTest {
    ...
    @Test
    fun `sendMessageWithType() with id should return a Message object with given id`() {
        mockServer.expect()
                .post()
                .withPath("/api/v1/chat.sendMessage")
                .andReturn(200, SEND_MESSAGE_WITH_TYPE_ID_OK)
                .once()

        runBlocking {
            val msg = sut.sendMessageWithType(
                    messageId = "1abbbf94-c839-4436-9476-6de03011c1e0",
                    roomId = "GENERAL",
                    messageType = "jitsiAudio",
                    message = "Sending message from SDK to #general and @here",
                    alias = "TestingAlias",
                    emoji = ":smirk:",
                    avatar = "https://avatars2.githubusercontent.com/u/224255?s=88&v=4")
